...
class StockInvestmentAI:
    def __init__(self, symbols=None, period='20y'):
        """
        Initialize the Stock Investment AI

        Parameters:
        symbols (list): List of stock symbols to analyze
        period (str): Time period for data collection ('5y', '20y', 'max' for maximum available data)
        """
        self.period = period
        # Increased to 500 stocks for more samples
        self.symbols = symbols or self.get_sp500_symbols()[:500]
        self.stock_data = {}
        ...
        self.recommendations = pd.DataFrame()  # Store as DataFrame
        self.horizon_recommendations = {}  # Store recommendations for each horizon
        self.market_regimes = {}

        # Persistent learning attributes
        self.model_dir = os.path.join(
            os.path.dirname(__file__), 'saved_models')
        self.history_dir = os.path.join(
            os.path.dirname(__file__), 'learning_history')
        self.performance_log = os.path.join(
            self.history_dir, 'performance_log.json')
        self.predictions_history = os.path.join(
            self.history_dir, 'predictions_history.csv')
        self.training_log = os.path.join(self.history_dir, 'training_log.json')
        ...
        self.performance_metrics = self._load_performance_log()
        self.previous_predictions = self._load_predictions_history()
    ...
    def get_sp500_symbols(self):
        ...
        return [
            ...
            'FMS', 'TDOC', 'PTON', 'CERN', 'ATHM', 'DOCS', 'VEEV', 'MTCH', 'TMDX',
    ...
    def _load_performance_log(self):
        """Load performance metrics from previous runs"""
    ...
    def _load_predictions_history(self):
        """Load historical predictions from previous runs"""
        if os.path.exists(self.predictions_history):
            try:
                return pd.read_csv(self.predictions_history)
            except:
                return pd.DataFrame()
        return pd.DataFrame()

    def _save_predictions_history(self, new_predictions):
        """Append new predictions to historical records"""
        try:
            if os.path.exists(self.predictions_history):
                existing = pd.read_csv(self.predictions_history)
                updated = pd.concat(
                    [existing, new_predictions], ignore_index=True)
                updated = updated.drop_duplicates(
                    subset=['Symbol', 'Date'], keep='last')
            else:
                updated = new_predictions

            updated.to_csv(self.predictions_history, index=False)
        except Exception as e:
            print(f"Warning: Could not save predictions history: {e}")

    def _load_training_log(self):
        """Load training history from previous runs"""
    ...
    def save_model(self, horizon):
        """Save trained models and scaler to disk"""
        try:
            model_path = os.path.join(
                self.model_dir, f'ensemble_model_{horizon}.joblib')
            scaler_path = os.path.join(
                self.model_dir, f'scaler_{horizon}.joblib')
            feature_selector_path = os.path.join(
                self.model_dir, f'feature_selector_{horizon}.joblib')
            imputer_path = os.path.join(
                self.model_dir, f'imputer_{horizon}.joblib')
            feature_names_path = os.path.join(
                self.model_dir, f'feature_names_{horizon}.joblib')

            # Save models
            joblib.dump(self.models, model_path)
            joblib.dump(self.scaler, scaler_path)

            # Save feature selector and imputer if they exist
            ...
            if hasattr(self, 'training_feature_names') and self.training_feature_names:
                joblib.dump(self.training_feature_names, feature_names_path)

            print(f"✅ Models saved for horizon {horizon}")
            return True
        except Exception as e:
            print(f"Warning: Could not save model for {horizon}: {e}")
            return False
    ...
    def load_model(self, horizon):
        ...
        try:
            ...
            # Load feature selector and imputer if they exist
            if os.path.exists(feature_selector_path):
                self.feature_selector = joblib.load(feature_selector_path)
            if os.path.exists(imputer_path):
                self.imputer = joblib.load(imputer_path)
            if os.path.exists(feature_names_path):
                self.training_feature_names = joblib.load(feature_names_path)

            print(f"✅ Loaded previously trained models for horizon {horizon}")
            return True
        except Exception as e:
            print(f"Note: Could not load model for {horizon}: {e}")
            return False

    def record_prediction(self, symbol, predicted_return, actual_return=None, horizon='20D', confidence=0.0):
        """Record a prediction for learning and validation"""
        try:
            prediction_record = {
                'Symbol': symbol,
                ...
                'Accuracy': (actual_return - predicted_return) ** 2 if actual_return else None
            }
    ...
    def log_training_session(self, horizon, model_performance, num_samples):
        """Log a training session for learning tracking"""
        try:
            session = {
            ...
            print(f"Warning: Could not log training session: {e}")

    def get_model_improvement_metrics(self):
        """Get metrics showing how models have improved over time"""
        if not self.training_history:
            return None

        metrics = {
            'total_training_sessions': len(self.training_history),
            'horizons_trained': list(set([s['horizon'] for s in self.training_history])),
            'total_samples_processed': sum([s.get('num_samples', 0) for s in self.training_history]),
            'training_sessions': self.training_history[-5:]  # Last 5 sessions
        }
        return metrics

    # ============================================================================
    # OPTIMIZED METHODS - EFFICIENCY IMPROVEMENT #1: Parallel Data Fetching
    # ============================================================================
    ...
    def fetch_stock_data_parallel(self):
        """
        OPTIMIZATION #1: Fetch stock data in parallel
    ...
    # ============================================================================
    # OPTIMIZED METHODS - EFFICIENCY IMPROVEMENT #2: Calculate Features Once
    # ============================================================================

    def create_features_dataset_optimized(self):
        """
        OPTIMIZATION #2: Calculate indicators ONCE per stock
        ...
        for symbol, data in self.stock_data.items():
            try:
                ...
                # Now extract features for lookback periods from the SAME df
                ...
                for lookback in lookback_periods:
                    ...
                    for horizon in [1, 5, 20, 60, 365, 730]:
    ...
    # ============================================================================
    # OPTIMIZED METHODS - ACCURACY IMPROVEMENT #3: No Data Leakage
    # ============================================================================

    def create_features_dataset_no_leakage(self):
        """
        OPTIMIZATION #3: Ensure NO FORWARD-LOOKING DATA LEAKAGE
    ...
    # ============================================================================
    # OPTIMIZED METHODS - ACCURACY IMPROVEMENT #4: Proper TimeSeriesSplit CV
    # ============================================================================

    def train_ml_model_proper_cv(self, target_column='Future_Return_365D'):
        """
        OPTIMIZATION #4: Proper time series cross-validation
    ...
    # ============================================================================
    # OPTIMIZED METHODS - ACCURACY IMPROVEMENT #5: Intelligent Feature Selection
    # ============================================================================
    ...
    # ============================================================================
    # OPTIMIZED METHODS - ACCURACY IMPROVEMENT #6: Better Imputation
    # ============================================================================

    def smart_impute(self, df):
        """
        OPTIMIZATION #6: Imputation that preserves time series properties
    ...
    def fetch_stock_data(self):
        """Fetch historical stock data for all symbols from multiple diverse data sources"""
        print(
            f"Fetching stock data for {len(self.symbols)} stocks from multiple data sources...")
        print("Sources: Yahoo Finance, Pandas DataReader, Alpha Vantage, Tiingo, and more")
        print("This may take several minutes due to the large number of stocks and 20 years of data...")
        ...
        for i, symbol in enumerate(self.symbols):
            try:
                ...
                if (i + 1) % 25 == 0:
                    print(
                        f"Progress: {i + 1}/{len(self.symbols)} stocks processed...")

                hist = None
                info = None
                data_source = None

                # Try primary source: Yahoo Finance (Ticker method)
                try:
                    stock = yf.Ticker(symbol)
                    hist = stock.history(period=self.period)
                    info = stock.info
                ...
                # Try secondary source: pandas_datareader with multiple backends
                if hist is None or (hasattr(hist, '__len__') and len(hist) < 100):
                    try:
                        ...
                        # Try Yahoo through pandas_datareader
                        try:
                            hist = pdr.get_data_yahoo(
                                symbol, start=start_date, end=end_date)
                            data_source = "PandasDataReader-Yahoo"
                        except:
                            # Try Google Finance through pandas_datareader
                            try:
                                hist = pdr.get_data_google(
                                    symbol, start=start_date, end=end_date)
                                data_source = "PandasDataReader-Google"
                            except:
                                pass

                        # Try to get additional info from yfinance if possible
                        try:
                            stock = yf.Ticker(symbol)
                            info = stock.info
                        except:
                ...
                # Try tertiary source: Yahoo Finance download method
                if hist is None or (hasattr(hist, '__len__') and len(hist) < 100):
                    try:
                        hist = yf.download(
                            symbol, period=self.period, progress=False, timeout=30)
                        ...
                        if hist is not None and hasattr(hist, '__len__') and len(hist) > 100:
                            self.stock_data[symbol] = {
                                'history': hist,
                                'info': info,
                                'data_source': data_source
                            ...
                            successful_stocks += 1
                            continue
                    except Exception as yfdown_error:
                        pass

                # Try quaternary source: Alpha Vantage (if key available)
                ...
                # Try quinary source: Tiingo (if key available)
                if hist is None or len(hist) < 100:
                    try:
                        import os
                        ...
                        if tiingo_key:
                            ...
                            hist = client.get_dataframe(symbol, metric='daily')
                            if len(hist) > 100:
                                # Ensure proper column names
                                hist = hist[['open', 'high',
                                             'low', 'close', 'volume']]
                                hist.columns = ['Open', 'High',
                                                'Low', 'Close', 'Volume']
                                data_source = "Tiingo"
                                info = {}
                ...
                print(f"✗ {symbol} - No data available from any source")
                failed_stocks += 1
        ...
        if self.stock_data:
            ...
            for source, count in sorted(sources.items(), key=lambda x: x[1], reverse=True):
                percentage = (count / len(self.stock_data)) * 100
                print(f"   • {source}: {count} stocks ({percentage:.1f}%)")
    ...
    def calculate_technical_indicators(self, df):
        ...
        for window in [3, 5, 8, 10, 13, 20, 21, 30, 50, 89, 100, 200]:
            ...
            if window <= 50:
                df[f'WMA_{window}'] = df['Close'].rolling(window=window).apply(
                    lambda x: (x * np.arange(1, len(x) + 1)).sum() /
                    np.arange(1, len(x) + 1).sum()
                )

        # Moving average relationships
        for short, long in [(5, 10), (5, 20), (10, 20), (20, 50), (50, 100), (50, 200), (100, 200)]:
            df[f'SMA_{short}_{long}_Ratio'] = df[f'SMA_{short}'] / \
                df[f'SMA_{long}']
            df[f'EMA_{short}_{long}_Ratio'] = df[f'EMA_{short}'] / \
                df[f'EMA_{long}']
            df[f'SMA_{short}_{long}_Cross'] = (
                df[f'SMA_{short}'] > df[f'SMA_{long}']).astype(int)

        # Price vs moving averages
        for window in [5, 10, 20, 50, 100, 200]:
            df[f'Price_SMA_{window}_Ratio'] = df['Close'] / df[f'SMA_{window}']
            df[f'Price_EMA_{window}_Ratio'] = df['Close'] / df[f'EMA_{window}']
            df[f'Price_Above_SMA_{window}'] = (
                df['Close'] > df[f'SMA_{window}']).astype(int)
        ...
        for fast, slow, signal in [(12, 26, 9), (5, 35, 5), (19, 39, 9), (8, 21, 5)]:
            ...
            df[f'MACD_Signal_{fast}_{slow}'] = df[f'MACD_{fast}_{slow}'].ewm(
                span=signal).mean()
            df[f'MACD_Hist_{fast}_{slow}'] = df[f'MACD_{fast}_{slow}'] - \
                df[f'MACD_Signal_{fast}_{slow}']
            df[f'MACD_Cross_{fast}_{slow}'] = (
                df[f'MACD_{fast}_{slow}'] > df[f'MACD_Signal_{fast}_{slow}']).astype(int)
        ...
        for window in [7, 14, 21, 30]:
            ...
            df[f'RSI_{window}_Overbought'] = (
                df[f'RSI_{window}'] > 70).astype(int)
            df[f'RSI_{window}_Oversold'] = (
                df[f'RSI_{window}'] < 30).astype(int)
        ...
        for window, std_dev in [(10, 1.5), (20, 2), (30, 2), (50, 2.5)]:
            ...
            df[f'BB_Width_{window}'] = (
                df[f'BB_Upper_{window}'] - df[f'BB_Lower_{window}']) / sma
            df[f'BB_Position_{window}'] = (df['Close'] - df[f'BB_Lower_{window}']) / (
                df[f'BB_Upper_{window}'] - df[f'BB_Lower_{window}'])
            df[f'BB_Squeeze_{window}'] = (
                df[f'BB_Width_{window}'] < df[f'BB_Width_{window}'].rolling(20).quantile(0.1)).astype(int)
        ...
        for k_window, d_window in [(14, 3), (21, 5), (5, 3)]:
            ...
            df[f'Stoch_D_{k_window}'] = df[f'Stoch_K_{k_window}'].rolling(
                window=d_window).mean()
            df[f'Stoch_Cross_{k_window}'] = (
                df[f'Stoch_K_{k_window}'] > df[f'Stoch_D_{k_window}']).astype(int)
        ...
        for window in [5, 10, 20, 30, 50]:
            df[f'Volume_SMA_{window}'] = df['Volume'].rolling(
                window=window).mean()
            df[f'Volume_Ratio_{window}'] = df['Volume'] / \
        ...
        # Price-Volume Trend
        df['PVT'] = ((df['Close'].diff() / df['Close'].shift(1))
                     * df['Volume']).fillna(0).cumsum()

        # Volume-Price Correlation
        for window in [10, 20, 30]:
            df[f'Price_Volume_Corr_{window}'] = df['Close'].rolling(
                window=window).corr(df['Volume'])
        ...
        for window in [14, 21]:
            ...
            positive_flow = money_flow.where(
                typical_price > typical_price.shift(1), 0).rolling(window=window).sum()
            negative_flow = money_flow.where(
                typical_price < typical_price.shift(1), 0).rolling(window=window).sum()
            df[f'MFI_{window}'] = 100 - \
                (100 / (1 + positive_flow / negative_flow))
        ...
        for window in [5, 10, 20, 30, 60, 90]:
            ...
            df[f'Volatility_Ann_{window}'] = df[f'Volatility_{window}'] * \
                np.sqrt(252)
        ...
        for window in [10, 20, 50]:
            df[f'Resistance_{window}'] = df['High'].rolling(
                window=window).max()
        ...
        df['Fib_23_6'] = df['Support_50'] + 0.236 * \
            (df['Resistance_50'] - df['Support_50'])
        df['Fib_38_2'] = df['Support_50'] + 0.382 * \
            (df['Resistance_50'] - df['Support_50'])
        df['Fib_61_8'] = df['Support_50'] + 0.618 * \
            (df['Resistance_50'] - df['Support_50'])

        # Ichimoku Cloud components
        tenkan_sen = (df['High'].rolling(9).max() +
                      df['Low'].rolling(9).min()) / 2
        kijun_sen = (df['High'].rolling(26).max() +
        ...
        df['Ichimoku_Senkou_B'] = (
            (df['High'].rolling(52).max() + df['Low'].rolling(52).min()) / 2).shift(26)
        ...
        # Aroon indicators
        for window in [14, 25]:
            df[f'Aroon_Up_{window}'] = (
                (window - df['High'].rolling(window).apply(lambda x: len(x) - 1 - np.argmax(x))) / window) * 100
            df[f'Aroon_Down_{window}'] = (
                (window - df['Low'].rolling(window).apply(lambda x: len(x) - 1 - np.argmin(x))) / window) * 100
            df[f'Aroon_Oscillator_{window}'] = df[f'Aroon_Up_{window}'] - \
                df[f'Aroon_Down_{window}']
    ...
    def calculate_fundamental_features(self, symbol, info):
        ...
        try:
            ...
            features['Price_to_Sales'] = info.get(
            ...
            features['Market_Cap_to_Revenue'] = info.get(
                'marketCap', 1) / max(info.get('totalRevenue', 1), 1)
            features['Book_Value_per_Share'] = info.get('bookValue', np.nan)
            features['Tangible_Book_Value'] = info.get(
                'tangibleBookValue', np.nan)
            features['Price_to_Tangible_Book'] = info.get(
                'currentPrice', 1) / max(info.get('tangibleBookValue', 1), 1)
            ...
            features['EPS_Growth'] = info.get(
                'earningsQuarterlyGrowth', np.nan)
            features['Revenue_Growth_Quarterly'] = info.get(
                'revenueQuarterlyGrowth', np.nan)
            features['EPS_Forward_Growth'] = info.get(
                'forwardEps', 0) - info.get('trailingEps', 0) if info.get('trailingEps') else np.nan
            features['Book_Value_Growth'] = info.get('bookValue', 0) / max(info.get(
                'priceToBook', 1) * info.get('currentPrice', 1), 1) if info.get('priceToBook') else np.nan
            features['Total_Debt_Growth'] = info.get(
                'totalDebt', 0) / max(info.get('totalCash', 1), 1)
            features['Asset_Growth'] = info.get(
                'totalAssets', 0) / max(info.get('marketCap', 1), 1)
            features['EBITDA_Growth'] = info.get(
                'ebitda', 0) / max(info.get('totalRevenue', 1), 1)
            features['Operating_Income_Growth'] = info.get(
                'operatingIncome', 0) / max(info.get('totalRevenue', 1), 1)
            features['Net_Income_Growth'] = info.get(
                'netIncomeToCommon', 0) / max(info.get('totalRevenue', 1), 1)
            features['Free_Cash_Flow_Growth'] = info.get(
                'freeCashflow', 0) / max(info.get('totalRevenue', 1), 1)
            ...
            features['Cash_Ratio'] = info.get(
                'totalCash', 0) / max(info.get('totalCurrentLiabilities', 1), 1)
            ...
            features['Interest_Coverage'] = info.get(
                'interestCoverage', np.nan)
            features['Debt_Service_Coverage'] = info.get(
                'totalDebt', 0) / max(info.get('operatingCashflow', 1), 1)
            features['Asset_Turnover'] = info.get(
                'totalRevenue', 0) / max(info.get('totalAssets', 1), 1)
            features['Inventory_Turnover'] = info.get(
                'totalRevenue', 0) / max(info.get('inventory', 1), 1)
            features['Receivables_Turnover'] = info.get(
                'totalRevenue', 0) / max(info.get('accountsReceivable', 1), 1)
            features['Working_Capital'] = info.get('workingCapital', np.nan)
            features['Working_Capital_Ratio'] = info.get(
                'workingCapital', 0) / max(info.get('totalRevenue', 1), 1)

            # Cash flow metrics (8 features)
            features['Free_Cash_Flow'] = info.get('freeCashflow', np.nan)
            features['Operating_Cash_Flow'] = info.get(
                'operatingCashflow', np.nan)
            features['FCF_Yield'] = info.get(
                'freeCashflow', 0) / max(info.get('marketCap', 1), 1)
            features['OCF_Yield'] = info.get(
                'operatingCashflow', 0) / max(info.get('marketCap', 1), 1)
            features['Cash_Conversion_Cycle'] = info.get(
                'cashConversionCycle', np.nan)
            features['FCF_to_Revenue'] = info.get(
                'freeCashflow', 0) / max(info.get('totalRevenue', 1), 1)
            features['OCF_to_Revenue'] = info.get(
                'operatingCashflow', 0) / max(info.get('totalRevenue', 1), 1)
            features['Capex_to_Revenue'] = info.get(
                'capitalExpenditures', 0) / max(info.get('totalRevenue', 1), 1)

            # Market & trading metrics (15 features)
            features['Market_Cap'] = info.get('marketCap', np.nan)
            features['Enterprise_Value_Full'] = info.get(
            ...
            features['Dividend_Coverage'] = info.get('trailingEps', 1) / max(
                info.get('dividendRate', 1), 1) if info.get('dividendRate') else np.nan
            features['Shares_Outstanding'] = info.get(
            ...
            features['Short_Percent_Float'] = info.get(
                'shortPercentOfFloat', np.nan)
            features['Institutional_Holdings'] = info.get(
                'heldPercentInstitutions', np.nan)
            features['Insider_Holdings'] = info.get(
                'heldPercentInsiders', np.nan)

            # Analyst & sentiment metrics (8 features)
            features['Recommendation_Mean'] = info.get(
                'recommendationMean', np.nan)
            ...
            features['Analyst_Count'] = info.get(
            ...
            features['Surprise_Percent'] = info.get(
                'earningsSurprisePercent', np.nan)

            # Business & operational metrics (10 features)
            features['Employee_Count'] = info.get('fullTimeEmployees', np.nan)
            features['Revenue_per_Employee'] = info.get(
                'totalRevenue', 0) / max(info.get('fullTimeEmployees', 1), 1)
            features['Profit_per_Employee'] = info.get(
                'netIncomeToCommon', 0) / max(info.get('fullTimeEmployees', 1), 1)
            features['Market_Cap_per_Employee'] = info.get(
                'marketCap', 0) / max(info.get('fullTimeEmployees', 1), 1)
            features['Sector'] = info.get('sector', 'Unknown')
            features['Industry'] = info.get('industry', 'Unknown')
            features['Business_Age'] = 2025 - \
                info.get('yearFirstAdded', 2025) if info.get(
                    'yearFirstAdded') else np.nan
            features['Market_Cap_Rank'] = 1 / \
                max(info.get('marketCap', 1), 1)  # Inverse for ranking
            features['Revenue_Rank'] = 1 / \
                max(info.get('totalRevenue', 1), 1)  # Inverse for ranking
            features['Employee_Efficiency'] = info.get(
                'totalRevenue', 0) / max(info.get('fullTimeEmployees', 1), 1)
            ...
            features['Price_to_52W_High'] = current_price / \
                max(info.get('fiftyTwoWeekHigh', 1), 1)
            features['Price_to_52W_Low'] = current_price / \
                max(info.get('fiftyTwoWeekLow', 1), 1)
            features['52W_Range_Position'] = (current_price - info.get('fiftyTwoWeekLow', current_price)) / max(
                info.get('fiftyTwoWeekHigh', current_price) - info.get('fiftyTwoWeekLow', 0), 1)
            features['Average_Volume'] = info.get('averageVolume', np.nan)
            features['Average_Volume_10D'] = info.get(
                'averageVolume10days', np.nan)
            features['Volume_Ratio_Avg'] = info.get(
                'volume', 1) / max(info.get('averageVolume', 1), 1)
            features['Price_Change_1D'] = (
                current_price - info.get('previousClose', current_price)) / max(info.get('previousClose', 1), 1)
            features['Market_Cap_Category'] = 1 if info.get('marketCap', 0) > 10e9 else 2 if info.get(
                'marketCap', 0) > 2e9 else 3  # Large, Mid, Small cap
            features['Volatility_52W'] = (info.get('fiftyTwoWeekHigh', 1) - info.get(
                'fiftyTwoWeekLow', 1)) / max(info.get('fiftyTwoWeekLow', 1), 1)
            features['Price_Momentum_Score'] = features['Price_to_52W_High'] * \
                features['52W_Change'] if features['52W_Change'] else np.nan
            features['Fundamental_Score'] = (features['ROE'] or 0) * (
                features['Revenue_Growth'] or 0) * (1 / max(features['PE_Ratio'] or 20, 1))
            features['Quality_Score'] = (
                features['ROE'] or 0) + (features['ROA'] or 0) + (features['ROIC'] or 0)

            # Calculate derived metrics and ratios
            if features['Target_Price'] and current_price > 0:
                features['Target_Upside'] = (
                    features['Target_Price'] - current_price) / current_price
                features['Target_Downside_Risk'] = (
                    current_price - features['Target_Low_Price']) / current_price if features['Target_Low_Price'] else np.nan
                features['Analyst_Confidence'] = 1 / max(abs(features['Target_High_Price'] - features['Target_Low_Price']) / features['Target_Price'], 0.01) if all([
                    features['Target_High_Price'], features['Target_Low_Price'], features['Target_Price']]) else np.nan

            # Risk-adjusted metrics
            features['Sharpe_Fundamental'] = (
                features['ROE'] or 0) / max(features['Beta'] or 1, 0.1)
            features['Quality_at_Price'] = (
                features['Quality_Score'] or 0) / max(features['PE_Ratio'] or 20, 1)
            features['Growth_at_Price'] = (
                features['Revenue_Growth'] or 0) / max(features['PEG_Ratio'] or 1, 0.1)

        except Exception as e:
            print(
                f"Error calculating comprehensive fundamentals for {symbol}: {e}")
    ...
    def calculate_volume_features(self, stock_data):
        ...
        try:
            ...
            features['Volume_SMA_10'] = df['Volume'].rolling(
                10).mean().iloc[-1]
            features['Volume_SMA_20'] = df['Volume'].rolling(
                20).mean().iloc[-1]
            features['Volume_Ratio_5'] = df['Volume'].iloc[-1] / \
                max(features['Volume_SMA_5'], 1)
            features['Volume_Ratio_10'] = df['Volume'].iloc[-1] / \
                max(features['Volume_SMA_10'], 1)
            features['Volume_Ratio_20'] = df['Volume'].iloc[-1] / \
                max(features['Volume_SMA_20'], 1)

            # Price-Volume relationships
            df['Price_Change'] = df['Close'].pct_change()
            df['Volume_Change'] = df['Volume'].pct_change()
            features['Price_Volume_Correlation'] = df['Price_Change'].rolling(
                20).corr(df['Volume_Change']).iloc[-1]

            # Volume trend analysis
            features['Volume_Trend_5'] = (features['Volume_SMA_5'] - df['Volume'].rolling(
                10).mean().iloc[-1]) / max(df['Volume'].rolling(10).mean().iloc[-1], 1)
            features['Volume_Trend_20'] = (features['Volume_SMA_20'] - df['Volume'].rolling(
                40).mean().iloc[-1]) / max(df['Volume'].rolling(40).mean().iloc[-1], 1)

            # On-Balance Volume
            df['OBV'] = (df['Volume'] * ((df['Close'] >
                         df['Close'].shift(1)).astype(int) * 2 - 1)).cumsum()
            features['OBV'] = df['OBV'].iloc[-1]
            features['OBV_Trend'] = df['OBV'].rolling(10).apply(
                lambda x: np.polyfit(range(len(x)), x, 1)[0]).iloc[-1]

            # Volume-Price Trend (VPT)
            df['VPT'] = (df['Volume'] * df['Close'].pct_change()).cumsum()
            features['VPT'] = df['VPT'].iloc[-1]
            features['VPT_Trend'] = df['VPT'].rolling(10).apply(
                lambda x: np.polyfit(range(len(x)), x, 1)[0]).iloc[-1]

            # Accumulation/Distribution Line
            df['AD_Line'] = ((df['Close'] - df['Low']) - (df['High'] -
                             df['Close'])) / (df['High'] - df['Low']) * df['Volume']
            df['AD_Line'] = df['AD_Line'].fillna(0).cumsum()
            features['AD_Line'] = df['AD_Line'].iloc[-1]
            features['AD_Line_Trend'] = df['AD_Line'].rolling(10).apply(
                lambda x: np.polyfit(range(len(x)), x, 1)[0]).iloc[-1]
    ...
    def calculate_timeframe_features(self, stock_data):
        ...
        try:
            ...
            for tf in timeframes:
                if len(df) > tf:
                    features[f'SMA_{tf}'] = df['Close'].rolling(
                        tf).mean().iloc[-1]
                    features[f'EMA_{tf}'] = df['Close'].ewm(
                        span=tf).mean().iloc[-1]
                    features[f'Price_SMA_{tf}_Ratio'] = df['Close'].iloc[-1] / \
                        max(features[f'SMA_{tf}'], 1)
                    features[f'Price_EMA_{tf}_Ratio'] = df['Close'].iloc[-1] / \
                        max(features[f'EMA_{tf}'], 1)

            # Moving average relationships
            if len(df) > 200:
                features['SMA_5_20_Ratio'] = features.get(
                    'SMA_5', 1) / max(features.get('SMA_20', 1), 1)
                features['SMA_20_50_Ratio'] = features.get(
                    'SMA_20', 1) / max(features.get('SMA_50', 1), 1)
                features['SMA_50_200_Ratio'] = features.get(
                    'SMA_50', 1) / max(features.get('SMA_200', 1), 1)

            # Golden/Death cross indicators
            features['Golden_Cross'] = 1 if features.get(
                'SMA_50', 0) > features.get('SMA_200', 0) else 0
            features['Death_Cross'] = 1 if features.get(
                'SMA_50', 0) < features.get('SMA_200', 0) else 0
    ...
    def calculate_correlation_features(self, stock_data, symbol):
        ...
        try:
            ...
            spy_data = yf.download(
                'SPY', start=stock_data.index[0], end=stock_data.index[-1], progress=False, timeout=30)
            if spy_data is not None and isinstance(spy_data, pd.DataFrame) and not spy_data.empty:
                ...
                if len(common_dates) > 20:
                    ...
                    features['SPY_Correlation'] = stock_aligned.corr(
                        spy_aligned)
                    features['SPY_Beta'] = stock_aligned.cov(
                        spy_aligned) / spy_aligned.var()

                    # Rolling correlations
                    rolling_corr = stock_aligned.rolling(20).corr(spy_aligned)
                    features['SPY_Correlation_20D'] = rolling_corr.iloc[-1]
                    features['SPY_Correlation_Trend'] = rolling_corr.rolling(10).apply(lambda x: np.polyfit(
                        range(len(x.dropna())), x.dropna(), 1)[0] if len(x.dropna()) > 1 else 0).iloc[-1]
    ...
    def calculate_statistical_features(self, stock_data):
        ...
        try:
            ...
            features['Expected_Shortfall'] = returns[returns <=
                                                     features['Value_at_Risk_5']].mean()
            features['Max_Drawdown'] = (
                (df['Close'] / df['Close'].cummax()) - 1).min()

            # Statistical tests
            from scipy import stats
            statistic, p_value = stats.jarque_bera(returns.dropna())
    ...
    def calculate_pattern_features(self, stock_data):
        ...
        try:
            ...
            df['Upper_Shadow'] = (
                df['High'] - df[['Close', 'Open']].max(axis=1)) / df['Close']
            df['Lower_Shadow'] = (df[['Close', 'Open']].min(
            ...
            features['Body_Upper_Ratio'] = features['Body_Size'] / \
                max(features['Upper_Shadow'], 0.001)
            features['Body_Lower_Ratio'] = features['Body_Size'] / \
                max(features['Lower_Shadow'], 0.001)
            ...
            features['Distance_to_Resistance'] = (
                highs.iloc[-1] - df['Close'].iloc[-1]) / df['Close'].iloc[-1]
            features['Distance_to_Support'] = (
                df['Close'].iloc[-1] - lows.iloc[-1]) / df['Close'].iloc[-1]
    ...
    def calculate_momentum_features(self, stock_data):
        ...
        try:
            ...
            # Momentum indicators
            features['Momentum_10'] = df['Close'].iloc[-1] - \
                df['Close'].iloc[-11] if len(df) > 10 else 0
            features['Momentum_20'] = df['Close'].iloc[-1] - \
                df['Close'].iloc[-21] if len(df) > 20 else 0
    ...
    def calculate_volatility_features(self, stock_data):
        ...
        try:
            ...
            features['Vol_Clustering'] = squared_returns.rolling(
                20).corr(squared_returns.shift(1)).iloc[-1]
    ...
    def calculate_seasonal_features(self, stock_data):
        ...
        try:
            ...
            last_day_of_month = calendar.monthrange(
                latest_date.year, latest_date.month)[1]
            ...
            # Year-end effects
            year_end_date = pd.Timestamp(f'{latest_date.year}-12-31')
            features['Days_to_Year_End'] = (year_end_date - latest_date).days
    ...
    def calculate_sentiment_features(self, symbol):
        """Calculate sentiment and market sentiment proxy features"""
        features = {}

        try:
            # Get recent price action for sentiment signals
            try:
                ticker = yf.Ticker(symbol)
                hist = yf.download(symbol, period='3mo',
                                   progress=False, timeout=30)

                # Price momentum sentiment (50-day SMA comparison)
                recent_price = float(hist['Close'].iloc[-1])
                sma_50_val = hist['Close'].rolling(50).mean().iloc[-1]
                sma_50 = float(
                    sma_50_val) if sma_50_val is not None else recent_price
                price_vs_sma_50 = (recent_price - sma_50) / \
                    sma_50 if sma_50 > 0 else 0

                # Map to 0-1 scale: strong negative (-5% or less) = 0, strong positive (+5% or more) = 1
                features['Market_Sentiment_Score'] = max(
                    min((price_vs_sma_50 + 0.05) / 0.10, 1), 0)

                # Volume sentiment
                volume_sma = hist['Volume'].rolling(20).mean()
                volume_sma_val = float(
                    volume_sma.iloc[-1]) if volume_sma.iloc[-1] is not None else 1
                volume_current = float(hist['Volume'].iloc[-1])
                volume_ratio = volume_current / \
                    volume_sma_val if volume_sma_val > 0 else 1
                features['Volume_Sentiment'] = min(
                    volume_ratio / 2, 1)  # Higher volume = more bullish

                # Volatility sentiment (lower vol = less fear)
                returns = hist['Close'].pct_change()
                vol_20 = float(returns.std()) * np.sqrt(252)
                vol_200_val = returns.rolling(200).std(
                ).iloc[-1] * np.sqrt(252) if len(returns) > 200 else vol_20
                vol_200 = float(
                    vol_200_val) if vol_200_val is not None else vol_20
                features['Volatility_Fear_Index'] = min(
                    vol_20 / max(vol_200, 0.1), 2) / 2  # Normalized to 0-1

                # Composite sentiment
                # High volatility = high fear
                features['Fear_Index'] = features['Volatility_Fear_Index']
                # Price momentum = greed
                features['Greed_Index'] = features['Market_Sentiment_Score']
                features['Market_Sentiment_VIX_Proxy'] = (
                    features['Fear_Index'] + (1 - features['Greed_Index'])) / 2

            except:
                # Fallback to neutral sentiment
                features['Market_Sentiment_Score'] = 0.5
                features['Volume_Sentiment'] = 0.5
                features['Volatility_Fear_Index'] = 0.5
                features['Fear_Index'] = 0.5
                features['Greed_Index'] = 0.5
                features['Market_Sentiment_VIX_Proxy'] = 0.5

            # Sector rotation indicators (based on technical positioning)
            features['Tech_Rotation_Score'] = 0.5  # Placeholder
            features['Value_Rotation_Score'] = 0.5  # Placeholder
            features['Defensive_Rotation_Score'] = 0.5  # Placeholder

        except Exception as e:
            print(f"Error calculating sentiment features for {symbol}: {e}")
            features['Market_Sentiment_Score'] = 0.5
            features['Volume_Sentiment'] = 0.5
            features['Volatility_Fear_Index'] = 0.5
            features['Fear_Index'] = 0.5
            features['Greed_Index'] = 0.5
            features['Market_Sentiment_VIX_Proxy'] = 0.5
            features['Tech_Rotation_Score'] = 0.5
            features['Value_Rotation_Score'] = 0.5
            features['Defensive_Rotation_Score'] = 0.5

        return features

    def calculate_options_flow_features(self, symbol):
        """Calculate options flow and implied volatility features"""
        features = {}
        ...
        try:
            ...
            # Implied volatility proxy based on options data
            try:
                options_dates = ticker.options
                if options_dates and len(options_dates) > 0:
                    # Use nearest term options (first expiration)
                    latest_option_date = options_dates[0]
                    try:
                        options_chain = ticker.option_chain(latest_option_date)
                        calls = options_chain.calls
                        puts = options_chain.puts

                        # Put-Call Ratio and volumes
                        if len(puts) > 0 and len(calls) > 0:
                            ...
                            features['Put_Call_Ratio'] = put_volume / \
                                max(call_volume, 1)
                            features['Put_Percentage'] = put_volume / \
                                max(total_volume, 1) if total_volume > 0 else 0.5

                            # Implied Volatility averages
                            call_iv_mean = calls['impliedVolatility'].fillna(
                                0).mean()
                            put_iv_mean = puts['impliedVolatility'].fillna(
                                0).mean()

                            features['Call_IV_Mean'] = call_iv_mean
                            features['Put_IV_Mean'] = put_iv_mean
                            features['IV_Spread'] = put_iv_mean - call_iv_mean
                            features['IV_Skew'] = abs(
                                features['IV_Spread']) / max((call_iv_mean + put_iv_mean) / 2, 0.001)

                            # Open Interest ratios
                            put_oi = puts['openInterest'].fillna(0).sum()
                            call_oi = calls['openInterest'].fillna(0).sum()
                            features['Put_Call_OI_Ratio'] = put_oi / \
                                max(call_oi, 1)

                        else:
                            features['Put_Call_Ratio'] = np.nan
                            features['Put_Percentage'] = 0.5
                            features['Call_IV_Mean'] = np.nan
                            features['Put_IV_Mean'] = np.nan
                            features['IV_Spread'] = np.nan
            ...
            print(f"Error calculating options flow features for {symbol}: {e}")
            features['Put_Call_Ratio'] = np.nan
            features['Put_Percentage'] = 0.5
            features['Call_IV_Mean'] = np.nan
            features['Put_IV_Mean'] = np.nan
    ...
    def calculate_macro_features(self):
        """Calculate macroeconomic indicator features from ETF proxies"""
        features = {}

        try:
            # Get current market data to derive macro proxies using ETFs
            macro_symbols = {
                'SPY': 'SPY_Momentum_30D',        # S&P 500 broad market
                'QQQ': 'Tech_Momentum_30D',      # Nasdaq tech
                'IEF': 'Bond_Yield_Proxy',       # 7-10Y Treasury
                'UUP': 'Dollar_Strength',        # USD Index proxy
                'GLD': 'Gold_Momentum',          # Gold safe-haven
                'USO': 'Oil_Momentum',           # Oil energy
                'EEM': 'Emerging_Markets',       # EM sentiment
                # High Yield bonds (junk bond risk)
                'HYG': 'Credit_Risk',
                'TLT': 'Long_Bond_Momentum',     # Long-term Treasuries
                'DBC': 'Commodity_Momentum'      # Broad commodity
            }

            for symbol, feature_name in macro_symbols.items():
                try:
                    data = yf.download(symbol, period='1mo',
                                       progress=False, timeout=30)['Close']
                    if data is not None and len(data) > 1:
                        current_price = float(data.iloc[-1])
                        prev_price = float(data.iloc[0])
                        if prev_price != 0:
                            momentum = (current_price -
                    ...
                    # Default to 0 for any download/parsing errors
                    features[feature_name] = 0

            # Risk-on/Risk-off indicator (Equity vs Bonds momentum ratio)
            spy_mom = features.get('SPY_Momentum_30D', 0)
            bond_mom = features.get('Bond_Yield_Proxy', 0)
            features['Risk_On_Score'] = max(
                min((spy_mom - bond_mom + 0.05) / 0.10, 1), 0)

            # Tech vs Value rotation
            tech_mom = features.get('Tech_Momentum_30D', 0)
            # Positive = tech outperforming
            features['Tech_vs_Value'] = tech_mom

            # Emerging Market vs Developed
            em_mom = features.get('Emerging_Markets', 0)
            features['Risk_Sentiment'] = max(
                min((em_mom + 0.05) / 0.10, 1), 0)  # EM outperformance = risk-on

            # Credit spreads proxy (junk vs treasuries)
            credit_risk = features.get('Credit_Risk', 0)
            # Negative credit momentum = higher spreads = stress
            features['Credit_Stress'] = credit_risk

            # Commodity cycle indicator
            commodity_mom = features.get('Commodity_Momentum', 0)
            features['Inflation_Expectation'] = max(
                min((commodity_mom + 0.05) / 0.10, 1), 0)

            # Duration and interest rate environment
            long_bond_mom = features.get('Long_Bond_Momentum', 0)
            # Negative correlation with falling yields
            features['Duration_Risk'] = -long_bond_mom

        except Exception as e:
            print(f"Error calculating macro features: {e}")
    ...
    def create_features_dataset(self):
        ...
        for symbol, data in self.stock_data.items():
            try:
                ...
                lookback_periods = [30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195,
                                    210, 225, 252, 300, 365, 400, 450, 500]  # 20 different lookback periods
                ...
                for lookback in lookback_periods:
                    ...
                    # Rolling statistics
                    volatility_20d = df['Close'].pct_change().rolling(
                        20).std().iloc[current_idx]
                    volatility_60d = df['Close'].pct_change().rolling(
                        60).std().iloc[current_idx]

                    # Sharpe ratio calculation
                    returns_60d_annualized = returns_60d * 252 / 60
                    sharpe_ratio = (returns_60d_annualized - 0.02) / \
                        (volatility_60d * np.sqrt(252)
                         ) if volatility_60d > 0 else 0

                    # Market regime indicators
                    vix_proxy = volatility_20d * 100  # VIX-like measure
                    trend_strength = abs(
                        current_data['SMA_20'] - current_data['SMA_50']) / current_data['Close']

                    # Calculate fundamental features (use latest available)
                    fundamental_features = self.calculate_fundamental_features(
                        symbol, info)

                    # Calculate advanced features from previously unused methods
                    # 200-day window for context
                    df_slice = df.iloc[max(0, current_idx-200):current_idx+1]
                    volume_features = self.calculate_volume_features(
                        df_slice) if len(df_slice) > 5 else {}
                    timeframe_features = self.calculate_timeframe_features(
                        df_slice) if len(df_slice) > 200 else {}
                    correlation_features = self.calculate_correlation_features(
                        df_slice, symbol) if len(df_slice) > 20 else {}
                    ...
                    volatility_features = self.calculate_volatility_features(
                        df_slice) if len(df_slice) > 60 else {}
                    seasonal_features = self.calculate_seasonal_features(
                        df_slice) if len(df_slice) > 0 else {}

                    # New advanced features (sentiment, options, macro)
                    sentiment_features = self.calculate_sentiment_features(
                        symbol)
                    options_features = self.calculate_options_flow_features(
                        symbol)
                    ...
                    targets = {}
                    # 1-day, 1-week, 2-week, 1-month, 3-month, 1-year, 2-year
                    prediction_horizons = [1, 5, 10, 20, 60, 365, 730]

                    for horizon in prediction_horizons:
                        future_idx = min(current_idx + horizon, len(df) - 1)
                        if future_idx > current_idx:
                            future_return = (
                                df['Close'].iloc[future_idx] - df['Close'].iloc[current_idx]) / df['Close'].iloc[current_idx]
        ...
        print(
            f"Created features dataset with {len(self.features_df)} samples and {len(self.features_df.columns)} features")

        # Handle missing values with advanced imputation
        print("Handling missing values...")

        # Separate numeric and categorical columns
        numeric_columns = self.features_df.select_dtypes(
        ...
        if len(numeric_columns) > 0:
            ...
            if numeric_imputed.shape[1] == len(numeric_columns):
                ...
                numeric_imputed_df = pd.DataFrame(
                    numeric_imputed, columns=numeric_columns, index=numeric_df.index)
                ...
                print(
                    f"Warning: Shape mismatch in imputation. Using median imputation instead.")
                # Fallback to simpler imputation
                for col in numeric_columns:
                    self.features_df[col] = self.features_df[col].fillna(
                        self.features_df[col].median())
                ...
                if col not in ['Symbol', 'Date_Index', 'Current_Price'] and col in self.features_df.columns:
                    try:
                        ...
                        if IQR > 0:  # Only process if IQR is positive
                            ...
                            self.features_df[col] = np.clip(
                                self.features_df[col], lower_bound, upper_bound)
                    except Exception as e:
                        print(
                            f"Warning: Could not process outliers for column {col}: {e}")
                        continue

        print(
            f"Final dataset: {len(self.features_df)} samples, {len(self.features_df.columns)} features")
        return self.features_df
    ...
    def train_ml_model(self, target_horizon='20D'):
        ...
        print(
            f"Training ensemble machine learning model for {target_horizon} horizon...")

        # Try to load previously trained model
        if self.load_model(target_horizon):
            print(f"ℹ️  Using previously trained models for faster predictions")
        ...
        if target_column not in self.features_df.columns:
            print(
                f"Target column {target_column} not found. Available targets:")
            target_cols = [
        ...
        exclude_columns = ['Symbol', 'Date_Index', 'Sector', 'Industry'] + \
            [col for col in self.features_df.columns if col.startswith(
                'Future_Return_')]

        feature_columns = [
            col for col in self.features_df.columns if col not in exclude_columns]
        ...
        print(
            f"Training on {len(valid_samples)} samples with {len(feature_columns)} features")
        ...
        X = X[valid_features]
        print(
            f"Retained {len(valid_features)} features after missing value filter")

        # Handle remaining missing values
        self.imputer = KNNImputer(n_neighbors=5)
        X_imputed = self.imputer.fit_transform(X)
        X_imputed = pd.DataFrame(
            X_imputed, columns=valid_features, index=X.index)
        ...
        selector = SelectKBest(score_func=f_regression,
                               k=min(100, len(valid_features)))
        X_selected = selector.fit_transform(X_imputed, y)
        selected_features = [valid_features[i]
        ...
        try:
            ...
            self.models['xgb'] = self.models['gb']  # Fallback to GB

        # Ridge Regression
        print("Training Ridge Regression...")
        self.models['ridge'] = Ridge(alpha=1.0)
        self.models['ridge'].fit(X_train_scaled, y_train)
        ridge_score = self.models['ridge'].score(X_test_scaled, y_test)
        ...
        print("Training SVR...")
        try:
            self.models['svr'] = SVR(C=1.0, gamma='scale', epsilon=0.1)
            self.models['svr'].fit(X_train_scaled, y_train)
            svr_score = self.models['svr'].score(X_test_scaled, y_test)
        ...
        try:
            self.models['mlp'] = MLPRegressor(hidden_layer_sizes=(
                100,), alpha=0.01, max_iter=500, random_state=42)
            self.models['mlp'].fit(X_train_scaled, y_train)
            mlp_score = self.models['mlp'].score(X_test_scaled, y_test)
        ...
        if len(ensemble_models) >= 2:
            ...
            cv_scores = cross_val_score(
                self.ensemble_model, X_train_scaled, y_train, cv=tscv, scoring='r2')
            print(
                f"CV R² Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
        ...
        # Save models for persistent learning
        self.save_model(target_horizon)

        # Log training session for performance tracking
        self.log_training_session(
            target_horizon, model_scores, len(valid_samples))

        return self.ensemble_model if self.ensemble_model else self.models.get('rf')

    def calculate_investment_score(self, target_horizon='365D'):
        ...
        # Load the horizon-specific trained model
        if not self.load_model(target_horizon):
            # If loading fails, train the model
        ...
        target_column = f'Future_Return_{target_horizon}'
        exclude_columns = ['Symbol', 'Date_Index', 'Sector', 'Industry'] + \
            [col for col in latest_data.columns if col.startswith(
                'Future_Return_')]

        feature_columns = [
            col for col in latest_data.columns if col not in exclude_columns]

        # Use only the features that were selected during training
        if hasattr(self, 'selected_features'):
            available_features = [
                col for col in self.selected_features if col in feature_columns]
        else:
        ...
        if hasattr(self, 'imputer') and hasattr(self, 'training_feature_names'):
            ...
            missing_features = set(
                self.training_feature_names) - set(X.columns)
        ...
        if self.ensemble_model:
            ...
            model_confidence = np.ones(
                len(predicted_returns))  # Default confidence
        else:
            # Use the best individual model
            best_model_name = 'rf'  # Default to Random Forest
            if self.models and len(self.models) > 0:
                # Find best model by trying to get score method
                best_score = -np.inf
                for model_name, model in self.models.items():
                    try:
                        if hasattr(model, 'score'):
                            # Get the number of samples (works for both dense and sparse)
                            n_samples = X_scaled.shape[0]
                            score = model.score(
                                X_scaled, np.ones(n_samples))
                            if score > best_score:
                                best_score = score
        ...
        for i, (idx, row) in enumerate(latest_data.iterrows()):
            ...
            for rsi_col in ['RSI', 'RSI_14', 'RSI_21']:
                if rsi_col in row and not pd.isna(row[rsi_col]):
                    ...
                    if 30 <= row[rsi_col] <= 70:
                        # Normalize around 50
                        rsi_score = 1 - abs(row[rsi_col] - 50) / 20
                    elif row[rsi_col] < 30:
            ...
            if not pd.isna(row.get('MACD', np.nan)) and not pd.isna(row.get('MACD_Signal', np.nan)):
                if row['MACD'] > row['MACD_Signal']:
                    macd_score = 0.7 + \
                        min(0.3, abs(row['MACD'] - row['MACD_Signal']) * 10)
                else:
                    macd_score = 0.3 - \
                        min(0.3, abs(row['MACD'] - row['MACD_Signal']) * 10)
            ...
            if not pd.isna(row.get('ROE', np.nan)):
                # 20% ROE is excellent
                roe_score = min(row['ROE'] / 0.20, 1) if row['ROE'] > 0 else 0
                fundamental_score += roe_score * 0.25
                fund_components += 0.25

            if not pd.isna(row.get('Profit_Margin', np.nan)):
                profit_score = min(
                    row['Profit_Margin'] / 0.15, 1) if row['Profit_Margin'] > 0 else 0
            ...
            if not pd.isna(row.get('Volatility_20D', np.nan)):
                ...
                if row['Volatility_20D'] <= 0.02:  # Very low volatility
                    ...
                    vol_score = max(
                        0.2, 1 - (row['Volatility_20D'] - 0.05) * 5)
            ...
            if not pd.isna(row.get('Sharpe_Ratio', np.nan)):
                sharpe_score = min(
                    max(row['Sharpe_Ratio'] / 2, 0), 1) if row['Sharpe_Ratio'] > 0 else 0
    ...
    def generate_recommendations(self, top_n=10, target_horizon='365D'):
        """Generate top stock recommendations using enhanced AI models"""
        scores_df = self.calculate_investment_score(target_horizon)

        # Debug: Show which horizon's data we're using
        top_5 = scores_df.head(5)[['Symbol', 'Expected_Return', 'Final_Score']]
        print(f"\n🎯 {target_horizon} horizon - Top 5 stocks by score (preview):")
        for idx, (_, row) in enumerate(top_5.iterrows(), 1):
            ret = row['Expected_Return'] * 100
            score = row['Final_Score']
            print(
                f"   {idx}. {row['Symbol']}: Score={score:.3f}, Expected Return={ret:+.1f}%")
        ...
        for idx, (_, stock) in enumerate(top_stocks.iterrows()):
            ...
            if not pd.isna(stock.get('Expected_Return')):
                ...
                print(
                    f"    {emoji} Expected Return ({target_horizon}): {expected_pct:+.1f}%")
            ...
            if not pd.isna(stock.get('Market_Cap')):
                ...
                print(
                    f"    📊 Market Cap: ${market_cap_b:.1f}B ({size_category})")
        ...
        if len(top_stocks) > 0:
            ...
            avg_confidence = top_stocks.get(
                'ML_Confidence', pd.Series([1.0] * len(top_stocks))).mean()
    ...
    def generate_investment_allocation_report(self, investment_amount=10000, risk_tolerance='moderate'):
        """Generate detailed investment allocation report with $10,000 allocation and reasoning"""
        if self.recommendations.empty:
            print("No recommendations available. Generating recommendations first...")
            self.generate_recommendations()

        print(f"\n{'='*80}")
        print("💰 AI INVESTMENT ALLOCATION - $10,000 PORTFOLIO RECOMMENDATION")
        print(f"{'='*80}")
        print(
            f"📅 Analysis Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"💵 Total Investment Capital: ${investment_amount:,.2f}")
        print(f"⚖️ Risk Tolerance: {risk_tolerance.upper()}")
        print(f"{'='*80}")
        ...
        if risk_tolerance == 'conservative':
            ...
            weights = np.array(
                [0.25, 0.20, 0.15, 0.12, 0.10, 0.08, 0.06, 0.04])
            risk_description = "Emphasis on established companies with lower volatility"
        elif risk_tolerance == 'moderate':
            top_n = 6
            weights = np.array([0.30, 0.25, 0.20, 0.15, 0.07, 0.03])
            risk_description = "Balanced blend of stable growth and opportunity plays"
        else:  # aggressive
            top_n = 4
            weights = np.array([0.40, 0.30, 0.20, 0.10])
            risk_description = "Concentrated positions in high-growth opportunities"
        ...
        print(f"   • Allocation Strategy: {risk_description}")
        print(
            f"   • Diversification Level: {'Conservative' if risk_tolerance == 'conservative' else 'Moderate' if risk_tolerance == 'moderate' else 'Concentrated'}")
        ...
        for sector, count in sorted(sector_dist.items(), key=lambda x: x[1], reverse=True):
            print(f"   • {sector}: {count} company/companies")

        print(f"\n{'='*80}")
        print("🎯 DETAILED ALLOCATION WITH INVESTMENT REASONING")
        print(f"{'='*80}")

        total_allocation = 0
        for rank, (_, stock) in enumerate(portfolio_stocks.iterrows(), 1):
            allocation_pct = weights[rank-1]
            allocation_amount = investment_amount * allocation_pct
            shares = int(allocation_amount / stock['Current_Price'])
            actual_amount = shares * stock['Current_Price']
            remaining_from_allocation = allocation_amount - actual_amount
            ...
            # Print allocation header
            print(
                f"\n{rank}. {stock['Symbol']} - ${actual_amount:,.2f} ({allocation_pct*100:.1f}%)")
            print(f"   {'─' * 75}")

            # Position details
            print(f"   💰 Position Details:")
            print(
                f"      • Shares: {shares} @ ${stock['Current_Price']:.2f}/share")
            ...
            print(f"      • Overall Score: {stock['Final_Score']:.3f} ⭐")
            print(
                f"      • AI Prediction Score: {stock['ML_Score']:.3f} (Expected Return: {stock.get('Expected_Return', 0)*100:+.1f}%)")
            print(
                f"      • Technical Score: {stock['Technical_Score']:.3f} (Chart patterns & indicators)")
            print(
                f"      • Fundamental Score: {stock['Fundamental_Score']:.3f} (Business quality)")
            print(
                f"      • Risk Score: {stock['Risk_Score']:.3f} (Volatility & beta)")

            # Key metrics
            print(f"\n   📈 Key Metrics:")
            metrics_display = []

            if not pd.isna(stock.get('PE_Ratio')) and stock['PE_Ratio'] > 0:
                metrics_display.append(f"P/E Ratio: {stock['PE_Ratio']:.1f}")
            if not pd.isna(stock.get('ROE')) and stock['ROE'] > 0:
                metrics_display.append(f"ROE: {stock['ROE']*100:.1f}%")
            if not pd.isna(stock.get('Beta')):
                metrics_display.append(f"Beta: {stock['Beta']:.2f}")
            if not pd.isna(stock.get('Dividend_Yield')) and stock['Dividend_Yield'] > 0:
                metrics_display.append(
                    f"Dividend Yield: {stock['Dividend_Yield']*100:.1f}%")
            ...
            # Investment reasoning
            print(
                f"\n   💡 Why We're Allocating {allocation_pct*100:.1f}% Here:")
            reasons = []
            ...
            if stock['ML_Score'] > 0.05:
                ...
                reasons.append(
                    f"Strong AI prediction model confidence - Expected 20-day return: {expected_return:+.1f}%")
            elif stock['ML_Score'] > 0.01:
                reasons.append(
                    f"Positive AI prediction with moderate confidence")
            else:
                reasons.append(f"AI model shows stabilization potential")

            # Technical analysis
            if stock['Technical_Score'] > 0.75:
                reasons.append(
                    f"Excellent technical signals with strong momentum")
            elif stock['Technical_Score'] > 0.6:
                reasons.append(
                    f"Positive technical indicators and price action")

            # Fundamental strength
            if stock['Fundamental_Score'] > 0.75:
                reasons.append(
                    f"Excellent fundamental metrics and business quality")
            elif stock['Fundamental_Score'] > 0.6:
                reasons.append(f"Solid fundamentals with attractive valuation")

            # Risk profile
            if stock['Risk_Score'] > 0.75:
                reasons.append(
                    f"Favorable risk/reward profile with controlled volatility")
            elif stock['Risk_Score'] > 0.6:
                reasons.append(
                    f"Acceptable risk level for this allocation size")

            # Market positioning
            if stock.get('Beta', 1.0) < 0.8:
                reasons.append(
                    f"Defensive positioning - less volatile than market")
            elif stock.get('Beta', 1.0) > 1.2:
                reasons.append(
                    f"Higher growth potential with above-market sensitivity")
            else:
                reasons.append(
                    f"Market-aligned volatility for balanced exposure")

            for i, reason in enumerate(reasons, 1):
                print(f"      {i}. {reason}")

            # Position rank reasoning
            print(f"\n   📊 Allocation Weight ({allocation_pct*100:.1f}%):")
            if rank == 1:
                print(
                    f"      → Highest conviction pick - best AI score & balanced fundamentals")
            elif rank == 2:
                print(f"      → Strong secondary position - complements primary holding")
            elif rank <= 4:
                print(f"      → Core portfolio position - proven strategy fit")
            elif rank <= 6:
                print(f"      → Supporting position - adds diversification")
            else:
                print(f"      → Tactical position - opportunistic allocation")

        # Summary and cash allocation
        remaining_cash = investment_amount - total_allocation
        ...
        print(f"{'='*80}")
        print(
            f"\nInvested in {top_n} stocks:  ${total_allocation:,.2f} ({total_allocation/investment_amount*100:.1f}%)")
        print(
            f"Reserved as cash buffer:    ${remaining_cash:,.2f} ({remaining_cash/investment_amount*100:.1f}%)")
        print(f"{'─' * 80}")
        print(f"Total allocation:           ${investment_amount:,.2f}")

        # Investment thesis summary
        print(f"\n{'='*80}")
        print("🎯 OVERALL INVESTMENT THESIS")
        print(f"{'='*80}")

        avg_score = portfolio_stocks['Final_Score'].mean()
        avg_expected_return = portfolio_stocks.get(
            'Expected_Return', pd.Series([0]*len(portfolio_stocks))).mean() * 100
        avg_beta = portfolio_stocks['Beta'].mean()
        avg_pe = portfolio_stocks['PE_Ratio'].mean()

        print(f"\n📊 Portfolio Metrics:")
        print(f"   • Average Investment Score: {avg_score:.3f}")
        print(
            f"   • Expected Portfolio Return (20-day): {avg_expected_return:+.1f}%")
        print(
            f"   • Average Beta: {avg_beta:.2f} ({'Conservative' if avg_beta < 0.8 else 'Balanced' if avg_beta <= 1.2 else 'Aggressive'})")
        print(f"   • Average P/E Ratio: {avg_pe:.1f}x")
        print(f"   • Number of Holdings: {top_n}")

        print(f"\n💼 Allocation Strategy Rationale:")
        print(
            f"   1. CONCENTRATION: {weights[0]*100:.0f}% in top pick provides maximum exposure to best opportunity")
        print(
            f"   2. CONVICTION: Top 3 picks ({sum(weights[:3])*100:.0f}%) represent core portfolio thesis")
        print(f"   3. DIVERSIFICATION: Remaining positions add sector & style variety")
        print(
            f"   4. LIQUIDITY BUFFER: {remaining_cash/investment_amount*100:.1f}% cash for rebalancing opportunities")

        print(f"\n🎯 Risk Management:")
        print(
            f"   • No single position exceeds {max(weights)*100:.1f}% to limit downside risk")
        print(f"   • Cash buffer allows for responding to market opportunities")
        print(
            f"   • Portfolio beta of {avg_beta:.2f}x {'provides defensive protection' if avg_beta < 1 else 'offers growth exposure'}")
        print(f"   • Sector diversification reduces company-specific risk")

        print(f"\n{'='*80}")
        print("✅ ALLOCATION COMPLETE - Ready for Implementation")
        print(f"{'='*80}\n")
    ...
    def plot_analysis(self):
        ...
        score_types = ['ML_Score', 'Technical_Score',
                       'Fundamental_Score', 'Risk_Score']
        ...
        ax4.hist(market_caps, bins=15, color='lightcoral',
    ...
    def run_analysis(self, investment_amount=10000, risk_tolerance='moderate', top_n=10, target_horizons=['365D', '730D']):
        ...
        # Show learning metrics from previous runs
        improvement_metrics = self.get_model_improvement_metrics()
        if improvement_metrics and improvement_metrics['total_training_sessions'] > 0:
            print(f"\n📚 LEARNING PROGRESS FROM PREVIOUS RUNS:")
            print(
                f"   Total training sessions: {improvement_metrics['total_training_sessions']}")
            print(
                f"   Horizons trained: {', '.join(improvement_metrics['horizons_trained'])}")
            print(
                f"   Total samples processed: {improvement_metrics['total_samples_processed']:,}")
        ...
        for horizon in target_horizons:
            print(f"\n🎯 Generating recommendations for {horizon} horizon...")
            horizon_recs = self.generate_recommendations(top_n, horizon)
            self.horizon_recommendations[horizon] = horizon_recs

        # Use the 1-year horizon for main portfolio allocation (or first available)
        main_horizon = '365D' if '365D' in target_horizons else target_horizons[0]
        recommendations = self.horizon_recommendations[main_horizon]
        ...
        print("="*60)
        portfolio = self.create_portfolio_allocation(
            investment_amount, risk_tolerance)

        # Step 5B: Generate detailed investment allocation report with $10,000 recommendation
        print("\n" + "="*60)
        print("💰 STEP 5B: GENERATING $10,000 INVESTMENT ALLOCATION REPORT")
        print("="*60)
        allocation_report = self.generate_investment_allocation_report(
            investment_amount, risk_tolerance)

        # Step 6: Create enhanced visualizations for all horizons
        print("\n" + "="*60)
        print("📊 STEP 6: CREATING ENHANCED VISUALIZATIONS")
        print("="*60)
        self.plot_all_horizon_dashboards(target_horizons)
        ...
        self.generate_analysis_report(
            recommendations, portfolio, model_performance)
    ...
    def generate_analysis_report(self, recommendations, portfolio, model_performance):
        ...
        print(
            f"Features Generated: {len(self.features_df.columns) if not self.features_df.empty else 0}")
        print(
            f"Training Samples: {len(self.features_df) if not self.features_df.empty else 0}")
        ...
        if not recommendations.empty:
            for idx, stock in recommendations.head(5).iterrows():
                ...
                expected_return = stock.get('Expected_Return', 0) * 100
                print(
                    f"   {idx+1}. {stock['Symbol']:>6} ({sector:>15}) | Score: {stock['Final_Score']:.3f} | Expected: {expected_return:+5.1f}%")
        ...
        if portfolio is not None and not portfolio.empty:
            ...
            for _, position in portfolio.iterrows():
                ...
                if symbol in recommendations['Symbol'].values:
                    sector = recommendations[recommendations['Symbol']
                                             == symbol]['Sector'].iloc[0]
                    sector_allocation[sector] = sector_allocation.get(
                        sector, 0) + position['Allocation_%']
        ...
        print(f"\n💡 KEY INSIGHTS:")
        print("   ✓ Enhanced AI models trained on 20 years of comprehensive data")
    ...
    def plot_enhanced_analysis(self, horizon='365D', recommendations=None):
        """Create enhanced visualization plots for a specific prediction horizon"""
        if recommendations is None:
            # Use stored recommendations for this horizon if available
            if horizon in self.horizon_recommendations:
                recommendations = self.horizon_recommendations[horizon]
            elif self.recommendations.empty:
                self.generate_recommendations()
        ...
        if isinstance(recommendations, pd.DataFrame) and recommendations.empty:
            print(f"No recommendations available for {horizon}")
            return
        ...
        bars = ax1.barh(range(len(top_15)),
                        top_15['Final_Score'], color='steelblue', alpha=0.8)
        ax1.set_yticks(range(len(top_15)))
        ax1.set_yticklabels(top_15['Symbol'])
        ax1.set_xlabel('Investment Score')
        ax1.set_title(f'🏆 Top 15 Stock Recommendations ({horizon} Horizon)',
        ...
        valid_conf = recommendations.dropna(
            subset=['ML_Confidence', 'Expected_Return'])
        ...
        for i, score_type in enumerate(score_types):
            values = top_8[score_type] if score_type in top_8.columns else [
                0] * len(top_8)
        ...
        valid_risk = recommendations.dropna(
            subset=['Beta', 'Expected_Return'])
        if not valid_risk.empty:
            sectors = valid_risk['Sector'].unique()
            colors_sector = plt.cm.get_cmap('Set3')(
                np.linspace(0, 1, len(sectors)))
        ...
        valid_mc = recommendations.dropna(subset=['Market_Cap'])
        if not valid_mc.empty:
            market_caps = valid_mc['Market_Cap'] / 1e9  # Convert to billions
            ax5.hist(market_caps, bins=12, color='lightcoral',
                     alpha=0.7, edgecolor='black')
        ...
        sector_counts = recommendations.head(20)['Sector'].value_counts()
        if not sector_counts.empty:
            colors_pie = plt.cm.get_cmap('Set3')(
                np.linspace(0, 1, len(sector_counts)))
            ax6.pie(sector_counts.values.tolist(), labels=sector_counts.index.tolist(), autopct='%1.1f%%',
                    startangle=90, colors=colors_pie.tolist())
            ax6.set_title('🏭 Top 20 Sector Distribution')

        # 7. PE Ratio vs Expected Return
        ax7 = fig.add_subplot(gs[2, 2])
        valid_pe = recommendations.dropna(
            subset=['PE_Ratio', 'Expected_Return'])
        if not valid_pe.empty:
            # Filter reasonable PE ratios
            valid_pe = valid_pe[(valid_pe['PE_Ratio'] > 0)
                                & (valid_pe['PE_Ratio'] < 100)]
            if not valid_pe.empty:
        ...
        plt.suptitle(f'🤖 AI-Powered Stock Investment Analysis Dashboard ({horizon} Prediction Horizon)',
                     fontsize=16, fontweight='bold', y=0.95)
        plt.show()

    def plot_all_horizon_dashboards(self, horizons):
        """Generate and display dashboards for all prediction horizons"""
        print(f"\n{'='*80}")
        print("📊 GENERATING DASHBOARDS FOR ALL PREDICTION HORIZONS")
        print(f"{'='*80}\n")

        for horizon in horizons:
            print(
                f"🎯 Displaying dashboard for {horizon} prediction horizon...")
            # Pass horizon-specific recommendations to the visualization
            horizon_recs = self.horizon_recommendations.get(horizon, None)
            self.plot_enhanced_analysis(
                horizon=horizon, recommendations=horizon_recs)
            print(f"✅ {horizon} dashboard displayed\n")
...
def main():
    ...
    print("=====================================")
    print("🔥 New Features:")
    print("   • 20 years of historical data for training")
    print("   • 80+ advanced technical & fundamental indicators")
    print("   • Ensemble ML with 6 different algorithms")
    print("   • Cross-validation with time series splits")
    print("   • Multiple prediction horizons (1D, 5D, 20D, 365D)")
    print("   • Advanced feature selection & outlier handling")
    print("   • Enhanced risk assessment & confidence scoring")
    print("   • 🆕 PERSISTENT LEARNING - Saves models between runs!")
    print("   • 🆕 Historical tracking - Learns from previous predictions")
    print("=====================================\n")

    # Initialize the enhanced AI
    ai = StockInvestmentAI(period='20y')  # Use 20 years of data
    ...
    recommendations, portfolio = ai.run_analysis(
        ...
        top_n=15,
        # Multiple prediction timeframes - view analysis for each horizon
    ...
    print(f"{'='*80}")
    print("✅ Comprehensive data analysis (20 years)")
    ...
    print(f"\n💡 KEY IMPROVEMENTS:")
    print("   🔹 Training data increased by 10x (2y → 20y)")
    ...
    print("   🔹 Models saved and reused across runs (learns from history!)")
    print("   🔹 Predictions tracked for continuous improvement")
